...
func register_tools(server_core: RefCounted) -> void:
	...
	_register_create_particles(server_core)
	_register_set_particle_material(server_core)
	_register_set_particle_color_gradient(server_core)
	_register_apply_particle_preset(server_core)
	_register_get_particle_info(server_core)
...
func _parse_color_value(color_str: String) -> Color:
	if color_str.begins_with("#"):
		return Color.html(color_str)
...
func _tool_create_particles(params: Dictionary) -> Dictionary:
	var scene_root: Node = _get_user_scene_root()
	if not scene_root:
		return {"error": "No scene is currently open"}
	var parent_path: String = String(params.get("parent_path", "."))
	...
	var node_name: String = String(params.get("name", "Particles"))
	var is_3d: bool = bool(params.get("is_3d", false))
	var amount: int = int(params.get("amount", 16))
	var lifetime: float = float(params.get("lifetime", 1.0))
	var one_shot: bool = bool(params.get("one_shot", false))
	...
	var error: Dictionary = _add_child_with_undo(parent, particles_node, scene_root, "MCP: Create particles")
	if not error.is_empty():
		return error
...
func _tool_set_particle_material(params: Dictionary) -> Dictionary:
	var scene_root: Node = _get_user_scene_root()
	if not scene_root:
		return {"error": "No scene is currently open"}
	var node_path: String = String(params.get("node_path", ""))
	if node_path.is_empty():
		return {"error": "Missing required parameter: node_path"}
	var node: Node = _get_particles_node_any(node_path)
	if not node:
		return {"error": "GPUParticles2D/3D at '" + node_path + "' not found"}

	var old_mat: Variant = node.get("process_material")
	var mat: ParticleProcessMaterial
	if old_mat is ParticleProcessMaterial:
		mat = (old_mat as ParticleProcessMaterial).duplicate(true) as ParticleProcessMaterial
	else:
		mat = ParticleProcessMaterial.new()

	var changes: Array = []
	if params.has("direction"):
		var dir: Variant = params["direction"]
		if dir is Dictionary:
			mat.direction = Vector3(float(dir.get("x", 0)), float(dir.get("y", 0)), float(dir.get("z", 0)))
			changes.append("direction")
		elif dir is String:
			var expr := Expression.new()
			if expr.parse(String(dir)) == OK:
				var parsed: Variant = expr.execute()
				if parsed is Vector3:
	...
	if params.has("color"):
		mat.color = _parse_color_value(String(params["color"]))
		changes.append("color")
	if params.has("emission_shape"):
		var shape_str: String = String(params["emission_shape"]).to_lower()
		match shape_str:
			"point":
			...
			"box":
				...
				if params.has("emission_box_extents"):
					var ext: Variant = params["emission_box_extents"]
	...
	if not changes.is_empty():
		_set_node_property_with_undo(node, "process_material", mat, "MCP: Set particle material")
	return {"node_path": node_path, "changes": changes}
...
func _apply_particle_gradient(mat: ParticleProcessMaterial, stops: Array) -> void:
	var offsets := PackedFloat32Array()
	var colors := PackedColorArray()
	for stop: Variant in stops:
		if stop is Dictionary:
			offsets.append(float(stop.get("offset", 0.0)))
			colors.append(_parse_color_value(String(stop.get("color", "#ffffff"))))
...
func _tool_set_particle_color_gradient(params: Dictionary) -> Dictionary:
	var scene_root: Node = _get_user_scene_root()
	if not scene_root:
		return {"error": "No scene is currently open"}
	var node_path: String = String(params.get("node_path", ""))
	...
	if not params.has("stops") or not params["stops"] is Array:
		return {"error": "Missing required parameter: stops (array of {offset, color})"}
	...
	for stop: Variant in stops:
		if stop is Dictionary:
			...
			colors.append(_parse_color_value(String(stop.get("color", "#ffffff"))))
	if offsets.is_empty():
		return {"error": "stops array must contain {offset, color} dictionaries"}
	...
	_set_node_property_with_undo(node, "process_material", mat, "MCP: Set particle color gradient")

	return {"node_path": node_path, "stops_count": gradient.get_point_count()}
...
func _register_particle_state_undo(node: Node, old_state: Dictionary, new_state: Dictionary, action_name: String) -> void:
	...
	if not editor_interface:
		for property: String in new_state:
			node.set(property, new_state[property])
		return
	var undo_redo: EditorUndoRedoManager = editor_interface.get_editor_undo_redo()
	undo_redo.create_action(action_name)
	for property: String in new_state:
...
func _tool_apply_particle_preset(params: Dictionary) -> Dictionary:
	var scene_root: Node = _get_user_scene_root()
	if not scene_root:
		return {"error": "No scene is currently open"}
	var node_path: String = String(params.get("node_path", ""))
	if node_path.is_empty():
		return {"error": "Missing required parameter: node_path"}
	var preset: String = String(params.get("preset", "")).to_lower()
	if preset.is_empty():
	...
	var old_state: Dictionary = _capture_particle_state(node)
	var preset_state: Dictionary = {}
	var mat := ParticleProcessMaterial.new()
	...
	match preset:
		"explosion":
			...
			_apply_particle_gradient(mat, [
				{"offset": 0.0, "color": "white"},
				{"offset": 0.3, "color": "#ffcc33"},
				{"offset": 0.7, "color": "#ff4d00"},
		...
		"fire":
			...
			mat.scale_max = 1.5
			_apply_particle_gradient(mat, [
				{"offset": 0.0, "color": "#ffff80"},
				{"offset": 0.3, "color": "#ff9900"},
				{"offset": 0.7, "color": "#cc3300"},
				{"offset": 1.0, "color": "#33000000"},
			])
		"smoke":
			...
			mat.damping_max = 2.0
			_apply_particle_gradient(mat, [
				{"offset": 0.0, "color": "#80808099"},
				{"offset": 0.5, "color": "#9999994d"},
				{"offset": 1.0, "color": "#b3b3b300"},
			])
		"sparks":
			...
			_apply_particle_gradient(mat, [
				...
				{"offset": 0.5, "color": "#ffb333"},
				{"offset": 1.0, "color": "#ff4d0000"},
			])
		"rain":
		...
		"magic":
			...
			mat.damping_max = 2.0
			_apply_particle_gradient(mat, [
				{"offset": 0.0, "color": "#4d80ff"},
				{"offset": 0.25, "color": "#ff4dcc"},
				{"offset": 0.5, "color": "#4dff80"},
				{"offset": 0.75, "color": "#ffcc33"},
				{"offset": 1.0, "color": "#804dff00"},
			])
		"dust":
			...
			_apply_particle_gradient(mat, [
				{"offset": 0.0, "color": "#ccc0a600"},
				{"offset": 0.2, "color": "#ccc0a64d"},
				{"offset": 0.8, "color": "#ccc0a64d"},
				{"offset": 1.0, "color": "#ccc0a600"},
			])
		_:
			return {"error": "Unknown preset: '" + preset + "'. Valid presets: explosion, fire, smoke, sparks, rain, snow, magic, dust"}

	preset_state["process_material"] = mat
	_register_particle_state_undo(node, old_state, preset_state, "MCP: Apply particle preset")

	return {"node_path": node_path, "preset": preset, "applied": true}

func _tool_get_particle_info(params: Dictionary) -> Dictionary:
	var scene_root: Node = _get_user_scene_root()
	if not scene_root:
		return {"error": "No scene is currently open"}
	var node_path: String = String(params.get("node_path", ""))
	...
	if mat is ParticleProcessMaterial:
		var pmat: ParticleProcessMaterial = mat
		var mat_info: Dictionary = {
			"direction": str(pmat.direction),
			"spread": pmat.spread,
			"initial_velocity_min": pmat.initial_velocity_min,
			"initial_velocity_max": pmat.initial_velocity_max,
			"gravity": str(pmat.gravity),
			"scale_min": pmat.scale_min,
			"scale_max": pmat.scale_max,
			"color": str(pmat.color),
			"angular_velocity_min": pmat.angular_velocity_min,
			"angular_velocity_max": pmat.angular_velocity_max,
			"orbit_velocity_min": pmat.orbit_velocity_min,
			"orbit_velocity_max": pmat.orbit_velocity_max,
			"damping_min": pmat.damping_min,
			"damping_max": pmat.damping_max,
			"attractor_interaction_enabled": pmat.attractor_interaction_enabled,
		}
		var shape_name: String
		match pmat.emission_shape:
			...
			_: shape_name = "unknown(%d)" % pmat.emission_shape
		...
		match pmat.emission_shape:
			ParticleProcessMaterial.EMISSION_SHAPE_SPHERE, ParticleProcessMaterial.EMISSION_SHAPE_SPHERE_SURFACE:
				mat_info["emission_sphere_radius"] = pmat.emission_sphere_radius
			ParticleProcessMaterial.EMISSION_SHAPE_BOX:
				mat_info["emission_box_extents"] = str(pmat.emission_box_extents)
			ParticleProcessMaterial.EMISSION_SHAPE_RING:
				mat_info["emission_ring_radius"] = pmat.emission_ring_radius
				mat_info["emission_ring_inner_radius"] = pmat.emission_ring_inner_radius
				mat_info["emission_ring_height"] = pmat.emission_ring_height
		if pmat.color_ramp is GradientTexture1D:
			var grad_tex: GradientTexture1D = pmat.color_ramp
...
# ============================================================================
# Registration helpers (Batch 4 particles)
# ============================================================================

func _register_create_particles(server_core: RefCounted) -> void:
	server_core.register_tool(
		"create_particles",
		"Create a GPUParticles2D or GPUParticles3D node with amount, lifetime, one_shot, explosiveness, and randomness settings.",
		{
			"type": "object",
			"properties": {
				"parent_path": {"type": "string", "description": "Parent node path. Default '.' for scene root."},
				"name": {"type": "string", "description": "Particles node name. Default 'Particles'."},
				"is_3d": {"type": "boolean", "description": "Create GPUParticles3D. Default false (2D)."},
				"amount": {"type": "integer", "description": "Particle count. Default 16."},
				"lifetime": {"type": "number", "description": "Particle lifetime in seconds. Default 1.0."},
				"one_shot": {"type": "boolean", "description": "Emit once. Default false."},
				"explosiveness": {"type": "number", "description": "Burst factor. Default 0."},
				"randomness": {"type": "number", "description": "Emission randomness. Default 0."},
				"emitting": {"type": "boolean", "description": "Start emitting. Default true."}
			},
			"required": [],
			"additionalProperties": false
		},
		Callable(self, "_tool_create_particles"),
		{
			"type": "object",
			"properties": {
				"name": {"type": "string"},
...
func _register_set_particle_material(server_core: RefCounted) -> void:
	server_core.register_tool(
		"set_particle_material",
		"Configure the ParticleProcessMaterial of a GPUParticles node: direction, spread, velocity, gravity, scale, color, emission shape, damping.",
		{
			"type": "object",
			"properties": {
				"node_path": {"type": "string"},
				"direction": {"type": "string", "description": "Emission direction as {x,y,z} or 'Vector3(x,y,z)'."},
				"spread": {"type": "number", "description": "Spread angle in degrees."},
				"initial_velocity_min": {"type": "number"},
				"initial_velocity_max": {"type": "number"},
				"gravity": {"type": "string", "description": "Gravity as {x,y,z} or 'Vector3(x,y,z)'."},
				"scale_min": {"type": "number"},
				"scale_max": {"type": "number"},
				"color": {"type": "string", "description": "Base color as '#RRGGBB' or named color."},
				"emission_shape": {"type": "string", "description": "point, sphere, sphere_surface, box, or ring."},
				"emission_sphere_radius": {"type": "number"},
				"emission_box_extents": {"type": "object", "description": "{x,y,z} extents for box shape."},
				"emission_ring_radius": {"type": "number"},
				"emission_ring_inner_radius": {"type": "number"},
				"emission_ring_height": {"type": "number"},
				"angular_velocity_min": {"type": "number"},
				"angular_velocity_max": {"type": "number"},
				"orbit_velocity_min": {"type": "number"},
				"orbit_velocity_max": {"type": "number"},
				"damping_min": {"type": "number"},
				"damping_max": {"type": "number"},
				"attractor_interaction_enabled": {"type": "boolean"}
			},
			"required": ["node_path"],
			"additionalProperties": false
		},
		Callable(self, "_tool_set_particle_material"),
		{
			"type": "object",
			"properties": {
				"node_path": {"type": "string"},
...
func _register_set_particle_color_gradient(server_core: RefCounted) -> void:
	server_core.register_tool(
		"set_particle_color_gradient",
		"Set a color gradient on a GPUParticles node's material from an array of {offset, color} stops.",
		{
			"type": "object",
			"properties": {
				"node_path": {"type": "string"},
				"stops": {"type": "array", "description": "Array of {offset: float 0-1, color: '#RRGGBB'}."}
			},
			"required": ["node_path", "stops"],
			"additionalProperties": false
		},
		Callable(self, "_tool_set_particle_color_gradient"),
		{
			"type": "object",
			"properties": {
				"node_path": {"type": "string"},
				"stops_count": {"type": "integer"}
			}
		},
		{"readOnlyHint": false, "destructiveHint": false, "idempotentHint": true, "openWorldHint": false},
		"supplementary", "World"
...
func _register_apply_particle_preset(server_core: RefCounted) -> void:
	server_core.register_tool(
		"apply_particle_preset",
		"Apply a predefined particle preset: explosion, fire, smoke, sparks, rain, snow, magic, or dust.",
		{
			"type": "object",
			"properties": {
				"node_path": {"type": "string"},
				"preset": {"type": "string", "description": "explosion, fire, smoke, sparks, rain, snow, magic, or dust."}
			},
			"required": ["node_path", "preset"],
			"additionalProperties": false
		},
		Callable(self, "_tool_apply_particle_preset"),
		{
			"type": "object",
			"properties": {
				"node_path": {"type": "string"},
...
func _register_get_particle_info(server_core: RefCounted) -> void:
	server_core.register_tool(
		"get_particle_info",
		"Read GPUParticles node state: amount, lifetime, one_shot, explosiveness, randomness, emitting, and material properties.",
		{
			"type": "object",
			"properties": {
				"node_path": {"type": "string"}
		...
		Callable(self, "_tool_get_particle_info"),
		{
			"type": "object",
			"properties": {
				"node_path": {"type": "string"},
