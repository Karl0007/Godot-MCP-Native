...
或从 [GitHub Releases](https://github.com/Karl0007/Godot-MCP-Native/releases) 下载。

详见 [CLI README](https://github.com/Karl0007/Godot-MCP-Native/blob/main/cli/gdmcp/README.md)。
...
插件注册了 **278 个工具**，但 `tools/list` 默认只返回 **33 个核心工具**。其余 **245 个补充工具**默认禁用：打开 **MCP** 停靠面板（视图 > 底部面板 > MCP），在 **Tools** 列表中勾选需要的工具，状态持久化到 `user://mcp_tool_state.cfg`。

> 无头环境提示：手动编辑 `mcp_tool_state.cfg` 后必须重算 `[meta]` 段的 `checksum`（对 `tools/<key>=<value>` 行以 `\n` 连接计算 MD5），否则改动会被忽略。先关闭编辑器再编辑——退出时会重写该文件。

运行时工具需要 `MCPRuntimeProbe` 自动加载；插件启用时会自动写入 `project.godot`。
