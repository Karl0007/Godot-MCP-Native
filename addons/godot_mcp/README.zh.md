...
- **全面的工具集**�?78 个工具—�?3 核心 + 245 补充）：
  - **节点工具**�? 核心 + 11 高级）：创建、修改、管理场景节点，复制、移动、重命名，锚点预设，信号连接，组管理，批量操作，场景审计
  - **脚本工具**�? 核心 + 8 高级）：编辑、分析、创建、附加、验�?GDScript �?C# 文件，执行脚本，文件搜索，符号索引，定义和引用查�?
  - **场景工具**�? 核心 + 4 高级）：操作场景结构、保存场景、列�?打开/关闭场景标签页，项目场景列表
  - **编辑器工�?*�? 核心 + 12 高级）：控制编辑器功能、截图、信号检查、文件系统重载，节点/文件选择，导出管理，属性检查器
  - **调试工具**�? 核心 + 68 高级）：日志、调试会话、断点、栈�?变量读取、性能分析器、运行时探针，动�?音频/着色器/瓦片地图运行时控制，调试执行控制，场景就绪等�?
  - **项目工具**�? 核心 + 23 高级）：访问项目设置、列出资源、创建资源，运行测试、管理输入映射、检查自动加�?全局类，资源诊断与健康审�?

## 🖥�?gdmcp CLI

本插件包含配套的 **gdmcp CLI**，供编程代理（Codex、Claude Code 等）使用�?
相比 MCP 可减少约 30,000 tokens 的上下文消耗。在 MCP 停靠面板�?**CLI Tools** 标签页中安装�?
...
插件注册�?**278 个工�?*，但 `tools/list` 默认只返�?**33 个核心工�?*。其�?**245 个补充工�?*默认禁用：打开 **MCP** 停靠面板（视�?> 底部面板 > MCP），�?**Tools** 列表中勾选需要的工具，状态持久化�?`user://mcp_tool_state.cfg`�?

> 无头环境提示：手动编�?`mcp_tool_state.cfg` 后必须重�?`[meta]` 段的 `checksum`（对 `tools/<key>=<value>` 行以 `\n` 连接计算 MD5），否则改动会被忽略。先关闭编辑器再编辑——退出时会重写该文件�?

运行时工具需�?`MCPRuntimeProbe` 自动加载；插件启用时会自动写�?`project.godot`�?
