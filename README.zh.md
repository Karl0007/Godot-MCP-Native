...
[English Version](https://github.com/Karl0007/Godot-MCP-Native/blob/main/README.md)
...
- **全面的工具集**（278 个工具——33 核心 + 245 补充）：
  - **节点工具**（9 核心 + 17 高级）：创建、修改、管理场景节点，复制、移动、重命名，锚点预设，信号连接，组管理，批量操作，场景审计
  - **脚本工具**（7 核心 + 8 高级）：编辑、分析、创建、附加、验证 GDScript 文件，执行脚本，文件搜索，符号索引，定义和引用查找
  - **场景工具**（4 核心 + 10 高级）：操作场景结构、保存场景、列出/打开/关闭场景标签页，项目场景列表，播放/停止场景
  - **编辑器工具**（4 核心 + 26 高级）：控制编辑器功能、截图、信号检查、文件系统重载，节点/文件选择，导出管理，属性检查器，编辑器相机，插件重载，Android 部署
  - **调试工具**（3 核心 + 82 高级）：日志、调试会话、断点、栈帧/变量读取、性能分析器、运行时探针，测试编排、输入录制/回放、运行时 UI 检查，动画/音频/着色器/瓦片地图运行时控制
  - **项目工具**（3 核心 + 41 高级）：访问项目设置、列出资源、创建资源，运行测试、管理输入映射、检查自动加载/全局类，资源诊断，项目健康审计，脚本符号
  - **世界工具**（0 核心 + 22 高级）：3D 场景构建、物理、导航与粒子（网格、光照、环境、相机、GridMap、碰撞、物理层、射线、导航、GPU 粒子）
  - **媒体工具**（0 核心 + 39 高级）：动画、音频、主题/UI、着色器与 TileMap 编辑（轨道/关键帧/状态机、音频总线、主题覆盖、着色器、瓦片地图）
  - **自举工具**（3 核心）：服务器就绪状态（godot_status、godot_ensure_ready、get_server_info）

## 🖥️ gdmcp CLI（Agent 优先接口）

对于有 Shell 访问权限的编程代理（Codex、Claude Code、Cursor），**gdmcp CLI**
是与 Godot MCP 交互的推荐方式。CLI 不会将所有 278 个工具 Schema 加载到模型上下文中，
...
完整文档见 [CLI README](https://github.com/Karl0007/Godot-MCP-Native/blob/main/cli/gdmcp/README.md) 和 [CLI Reference](https://github.com/Karl0007/Godot-MCP-Native/blob/main/docs/current/gdmcp-cli-reference.md)。

**Skill**：从 [skills/gdmcp/](https://github.com/Karl0007/Godot-MCP-Native/blob/main/skills/gdmcp) 安装配套 Codex Skill，教你的代理使用 CLI 工作流。
将 [skills/gdmcp/](https://github.com/Karl0007/Godot-MCP-Native/blob/main/skills/gdmcp) 复制到 `~/.codex/skills/gdmcp/`。
...
**278 个工具**——33 核心 + 245 补充。核心工具默认启用；补充工具需要在 MCP 面板中启用（见「启用补充工具」）。
...
- [快速开始指南](https://github.com/Karl0007/Godot-MCP-Native/blob/main/docs/current/quickstart.md)
- [架构设计](https://github.com/Karl0007/Godot-MCP-Native/blob/main/docs/current/architecture.md)
- [工具参考](https://github.com/Karl0007/Godot-MCP-Native/blob/main/docs/current/tools-reference.md)
- [CLI 参考](https://github.com/Karl0007/Godot-MCP-Native/blob/main/docs/current/gdmcp-cli-reference.md)
- [发布流程](https://github.com/Karl0007/Godot-MCP-Native/blob/main/docs/development/release-workflow.md)
- [测试指南](https://github.com/Karl0007/Godot-MCP-Native/blob/main/docs/current/testing-guide.md)
...
本项目采用 MIT 许可证 - 详见 [LICENSE](https://github.com/Karl0007/Godot-MCP-Native/blob/main/LICENSE) 文件。
