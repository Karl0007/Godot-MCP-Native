...
### 227. add_scene_instance

实例化场景为子节点。

**参数**：`scene_path`（是）、`parent_path`（否，默认 `.`）、`name`（否）

**返回值**：`node_path`、`scene_path`、`name`

### 228. play_scene

播放场景。

**参数**：`mode`（否，main/current/场景路径）
...
### 231. get_scene_exports

列出场景中所有脚本节点的 @export 属性。

**参数**：`path`（是）

**返回值**：`path`、`nodes`（含 node_path/exports）、`count`
...
**参数**：`preset_index`/`preset_name`（二选一）、`debug`（否，默认 true）

**返回值**：`preset`、`export_path`、`command`、`message`
...
**返回值**：`has_export_presets`、`godot_executable`、`project_path`、`templates_dir`、`templates_installed`

**注解**：`readOnlyHint=true`, `destructiveHint=false`, `idempotentHint=true`, `openWorldHint=false`

### 248. list_android_devices

列出 adb 设备。

**参数**：无

**返回值**：`devices`、`count`、`adb_path`

**注解**：`readOnlyHint=true`, `destructiveHint=false`, `idempotentHint=true`, `openWorldHint=false`

### 249. get_android_preset_info

读取 Android 导出预设。

**参数**：`preset_name`/`preset_index`（二选一）

**返回值**：`index`、`name`、`platform`、`export_path`、`runnable`

**注解**：`readOnlyHint=true`, `destructiveHint=false`, `idempotentHint=true`, `openWorldHint=false`

### 250. deploy_to_android

导出并安装 APK。

**参数**：`preset_name`/`preset_index`（二选一）、`debug`（否）、`device_serial`（否）

**返回值**：`steps`、`success`、`export_path`
...
**参数**：`session_id`/`timeout_ms`（可选）

**返回值**：`recording`、`message`
...
**返回值**：`recording`、`events`（含 time_ms 时间线）、`event_count`、`duration_ms`

**注解**：`readOnlyHint=false`, `destructiveHint=false`, `idempotentHint=true`, `openWorldHint=true`

### 258. replay_recording

回放录制的事件序列。

**参数**：`events`（是）、`speed`（否，默认 1.0）、`session_id`/`timeout_ms`（可选）

**返回值**：`replayed`、`event_count`、`speed`
...
### 266. analyze_signal_flow

分析场景信号连接。

**参数**：`signal_name`（否）、`node_path`（否）

**返回值**：`connections`（含 source/signal/target/method）、`count`

**注解**：`readOnlyHint=true`, `destructiveHint=false`, `idempotentHint=true`, `openWorldHint=false`

### 267. analyze_scene_complexity

分析场景复杂度。

**参数**：`path`（否，默认当前场景）

**返回值**：`scene_path`、`total_nodes`、`max_depth`、`nodes_by_type`、`scripts_attached`、`issues`

**注解**：`readOnlyHint=true`, `destructiveHint=false`, `idempotentHint=true`, `openWorldHint=false`

### 268. detect_circular_dependencies

检测场景循环依赖（DFS）。

**参数**：`path`（否）、`include_addons`（否）

**返回值**：`scenes_checked`、`circular_dependencies`、`has_circular`

**注解**：`readOnlyHint=true`, `destructiveHint=false`, `idempotentHint=true`, `openWorldHint=false`

### 269. find_unused_resources

扫描未使用资源。

**参数**：`path`（否）、`include_addons`（否）

**返回值**：`unused`、`count`、`total_scanned`
