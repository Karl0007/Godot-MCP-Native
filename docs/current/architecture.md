...
Godot-MCP 实现了 **275 个工具**，分为 8 大类和两种级别：

**核心工具（30 个）**：默认启用，始终可见。
**补充工具（125 个）**：需在工具管理面板中手动启用。

| 类别 | 核心 | 补充 | 总计 | 说明 |
|------|------|------|------|------|
| Node Tools | 9 | 11 | 20 | 节点管理（创建、删除、修改属性、复制、移动、重命名、信号、组） |
| Script Tools | 7 | 7 | 14 | 脚本管理（读取、修改、创建、分析、符号索引、搜索） |
| Scene Tools | 4 | 4 | 8 | 场景管理（创建、保存、打开、结构查看） |
| Editor Tools | 4 | 12 | 16 | 编辑器操作（运行、停止、截图、导出、选择） |
| Debug Tools | 3 | 68 | 71 | 调试和运行时（日志、断点、栈帧、Profiler、运行时探针、动画/音频/着色器/瓦片地图控制） |
| Project Tools | 3 | 23 | 26 | 项目配置（信息、设置、资源、测试、输入映射、自动加载、全局类、诊断） |
